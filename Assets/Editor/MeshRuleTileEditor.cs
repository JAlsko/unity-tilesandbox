...
namespace UnityEditor
{
    ...
    internal class MeshRuleTileEditor : Editor
    {
        ...
        private MeshRuleTile tile { get { return (target as MeshRuleTile); } }
        ...
        public void OnEnable()
        {
            if (tile.m_TilingRules == null)
                tile.m_TilingRules = new List<MeshRuleTile.TilingRule>();

            m_ReorderableList = new ReorderableList(tile.m_TilingRules, typeof(MeshRuleTile.TilingRule), true, true, true, true);
        ...
        {
            MeshRuleTile.TilingRule rule = tile.m_TilingRules[index];
        ...
        {
            MeshRuleTile.TilingRule rule = new MeshRuleTile.TilingRule();
            rule.m_Output = MeshRuleTile.TilingRule.OutputSprite.Single;
        ...
        {
            tile.m_DefaultSprite = EditorGUILayout.ObjectField("Default Sprite", tile.m_DefaultSprite, typeof(Texture2D), false) as Texture2D;

            serializedObject.Update();
            EditorGUI.BeginChangeCheck();
            var baseFields = typeof(MeshRuleTile).GetFields().Select(field => field.Name);
        ...
        internal virtual void RuleOnGUI(Rect rect, int arrowIndex, int neighbor)
        {
            switch (neighbor)
            {
                case MeshRuleTile.TilingRule.Neighbor.DontCare:
                    break;
                case MeshRuleTile.TilingRule.Neighbor.This:
                    GUI.DrawTexture(rect, arrows[arrowIndex]);
                    break;
                case MeshRuleTile.TilingRule.Neighbor.NotThis:
        ...
        internal virtual void RuleTransformOnGUI(Rect rect, MeshRuleTile.TilingRule.Transform ruleTransform)
        {
            switch (ruleTransform)
            {
                case MeshRuleTile.TilingRule.Transform.Rotated:
                    GUI.DrawTexture(rect, autoTransforms[0]);
                    break;
                case MeshRuleTile.TilingRule.Transform.MirrorX:
                    GUI.DrawTexture(rect, autoTransforms[1]);
                    break;
                case MeshRuleTile.TilingRule.Transform.MirrorY:
                    GUI.DrawTexture(rect, autoTransforms[2]);
                    break;
            }
        }

        internal void RuleNeighborUpdate(Rect rect, MeshRuleTile.TilingRule tilingRule, int index)
        {
        ...
        internal void RuleTransformUpdate(Rect rect, MeshRuleTile.TilingRule tilingRule)
        {
            if (Event.current.type == EventType.MouseDown && ContainsMousePosition(rect))
            {
                tilingRule.m_RuleTransform = (MeshRuleTile.TilingRule.Transform)(int)Mathf.Repeat((int)tilingRule.m_RuleTransform + GetMouseChange(), 4);
                GUI.changed = true;
                Event.current.Use();
            }
        }
        ...
        internal virtual void RuleMatrixOnGUI(MeshRuleTile tile, Rect rect, MeshRuleTile.TilingRule tilingRule)
        {
        ...
        internal static void SpriteOnGUI(Rect rect, MeshRuleTile.TilingRule tilingRule)
        {
            tilingRule.m_Sprites[0] = EditorGUI.ObjectField(new Rect(rect.xMax - rect.height, rect.yMin, rect.height, rect.height), tilingRule.m_Sprites[0], typeof(Texture2D), false) as Texture2D;
        }

        internal static void RuleInspectorOnGUI(Rect rect, MeshRuleTile.TilingRule tilingRule)
        {
            float y = rect.yMin;
            EditorGUI.BeginChangeCheck();
            GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Rule");
            tilingRule.m_RuleTransform = (MeshRuleTile.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RuleTransform);
            y += k_SingleLineHeight;
            GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Output");
            tilingRule.m_Output = (MeshRuleTile.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
            y += k_SingleLineHeight;

            if (tilingRule.m_Output == MeshRuleTile.TilingRule.OutputSprite.Animation)
            {
            ...
            if (tilingRule.m_Output == MeshRuleTile.TilingRule.OutputSprite.Random)
            {
                ...
                tilingRule.m_RandomTransform = (MeshRuleTile.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RandomTransform);
                y += k_SingleLineHeight;
            }

            if (tilingRule.m_Output != MeshRuleTile.TilingRule.OutputSprite.Single)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                    ...
                    Array.Resize(ref tilingRule.m_SpriteAtlasIndices, Math.Max(newLength, 1));
                y += k_SingleLineHeight;

                for (int i = 0; i < tilingRule.m_Sprites.Length; i++)
                {
                    tilingRule.m_Sprites[i] = EditorGUI.ObjectField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Sprites[i], typeof(Texture2D), false) as Texture2D;
                    y += k_SingleLineHeight;
                }
            }
        }
        ...
        {
            [SerializeField]
            public List<MeshRuleTile.TilingRule> rules = new List<MeshRuleTile.TilingRule>();
        }

        [MenuItem("CONTEXT/RuleTile/Copy All Rules")]
        private static void CopyAllRules(MenuCommand item)
        {
            MeshRuleTile tile = item.context as MeshRuleTile;
            if (tile == null)
                return;
